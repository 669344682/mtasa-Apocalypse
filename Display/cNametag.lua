...
--((Developers: DorteY))
--((File: Display/cNametag.lua))

nameSphere=createColSphere(0,0,0,12)
...
function nameTagSphereHit(element)
	if(getElementType(element)=="player")then--Player see yourself name
							...
							if(nameTagVisible[i])then
								...
								if(x)and(y)and(z)then
									...
									if(sx)and(sy)then
										...
										if(getElementData(i,"Gruppe")~="none")then
											nametag="["..getElementData(i,"Gruppe").."] "
										end
										...
										for k,v in pairs(nameTagImages[i])do
											if(v)then
												local imgpath=":"..settings.generel.scriptname.."/Files/Images/Nametag/"..k
